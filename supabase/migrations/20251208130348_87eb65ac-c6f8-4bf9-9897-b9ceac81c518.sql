-- Drop and recreate checkout functions WITHOUT exposing seller contact info
DROP FUNCTION IF EXISTS get_product_for_checkout(UUID);
DROP FUNCTION IF EXISTS get_product_for_checkout_by_slug(TEXT);

-- Create function to get product for checkout by ID (without sensitive seller data)
...
-- Create function to get product for checkout by slug (without sensitive seller data)
