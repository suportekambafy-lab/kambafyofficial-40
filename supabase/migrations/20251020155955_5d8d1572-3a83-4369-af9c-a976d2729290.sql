...
-- Criar função para buscar aulas acessíveis por email de estudante
-- Essa função bypassa RLS usando SECURITY DEFINER e verifica sessões válidas
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE student_email = p_student_email
      AND member_area_id = p_member_area_id
      AND expires_at > NOW()
...
-- Grant execute to public (anon users can call it)
