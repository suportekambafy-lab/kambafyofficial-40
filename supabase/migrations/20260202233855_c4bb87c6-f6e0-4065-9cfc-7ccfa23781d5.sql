...
-- The issue: NEW.amount is TEXT but we're doing math with it
...
BEGIN
  ...
  -- Convert amount to numeric (handle both text and numeric types)
  BEGIN
    order_amount := NEW.amount::numeric;
  ...
  platform_fee_amount := order_amount - affiliate_commission_amount - seller_net_amount;
