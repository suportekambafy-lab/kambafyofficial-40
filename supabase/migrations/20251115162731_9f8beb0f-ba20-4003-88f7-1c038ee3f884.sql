-- Atualizar função de validação para lidar melhor com precisão de ponto flutuante
...
DECLARE
  ...
  v_rounded_available NUMERIC;
BEGIN
  -- Arredondar o valor da solicitação para 2 casas decimais
  v_rounded_amount := ROUND(NEW.amount::NUMERIC, 2);
  
  -- Buscar saldo total do usuário
  SELECT ROUND(balance::NUMERIC, 2) INTO v_total_balance
  ...
  -- Calcular valor retido e saldo disponível (arredondados)
  v_retained_amount := ROUND((v_total_balance * v_retention_percentage) / 100, 2);
  v_available_balance := ROUND(v_total_balance - v_retained_amount, 2);
  ...
  IF v_retention_percentage > 0 THEN
    ...
    IF v_retention_release_date IS NULL OR v_retention_release_date > NOW() THEN
      ...
      IF v_rounded_amount > v_available_balance THEN
    ...
    ELSE
      ...
      IF v_rounded_amount > v_total_balance THEN
        RAISE EXCEPTION 'Valor do saque (%) excede o saldo total (%).', v_rounded_amount, v_total_balance;
      END IF;
    END IF;
  ELSE
