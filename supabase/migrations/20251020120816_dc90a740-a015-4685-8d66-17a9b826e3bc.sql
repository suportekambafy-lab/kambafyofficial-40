-- ✅ REVERTER para política anterior que mostrava as aulas
...
USING (
  ...
  AND (
    ...
    OR
    -- Permite acesso se a aula pertence a uma área de membros existente
    EXISTS (
      SELECT 1 FROM member_areas ma
      WHERE ma.id = lessons.member_area_id
    )
  ...
  AND (
    -- Verificação básica de módulos pagos
