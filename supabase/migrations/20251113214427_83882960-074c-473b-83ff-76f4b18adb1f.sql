-- Atualizar função para corrigir TODAS as transferências sem taxa
...
BEGIN
  ...
  FOR transfer_record IN 
    ...
    WHERE o.payment_method IN ('transfer', 'bank_transfer', 'transferencia')
      ...
      AND bt.type = 'credit'
      -- Buscar por QUALQUER descrição de venda (não apenas "Venda do produto")
      AND (bt.description LIKE '%Venda%' OR bt.description LIKE '%venda%')
      ...
      AND NOT EXISTS (
        ...
        AND (bt2.description LIKE '%Correção de taxa%' OR bt2.description LIKE '%Taxa da plataforma%')
      )
