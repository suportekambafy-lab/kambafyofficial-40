...
-- 40. get_pending_transfers_for_admin
CREATE OR REPLACE FUNCTION public.get_pending_transfers_for_admin()
RETURNS TABLE(id uuid, order_id text, customer_name text, customer_email text, amount text, currency text, created_at timestamp with time zone, payment_proof_data jsonb, user_id uuid, status text, payment_method text, product_name text)
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
...
-- 42. check_customer_access
CREATE OR REPLACE FUNCTION public.check_customer_access(p_customer_email text, p_product_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SET search_path = public
...
-- 44. count_duplicate_withdrawals
...
-- Recriar funções auxiliares (já têm search_path mas vou garantir)
-- Estas já foram feitas mas vou confirmar

-- 46. calculate_access_expiration (JÁ TEM search_path)
...
COMMENT ON FUNCTION public.get_seller_stats IS 'Retorna estatísticas do vendedor - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.get_all_identity_verifications_for_admin IS 'Lista todas as verificações de identidade (apenas admins) - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.get_all_withdrawal_requests_for_admin IS 'Lista todos os pedidos de saque (apenas admins) - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.get_pending_transfers_for_admin IS 'Lista transferências pendentes (apenas admins) - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.get_order_details_for_admin IS 'Detalhes completos de pedidos (apenas admins) - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.check_customer_access IS 'Verifica se cliente tem acesso a produto - search_path configurado';
COMMENT ON FUNCTION public.calculate_commissions IS 'Calcula comissões de vendas - search_path configurado';
COMMENT ON FUNCTION public.count_duplicate_withdrawals IS 'Conta saques duplicados (apenas admins) - SECURITY DEFINER com search_path';
COMMENT ON FUNCTION public.is_current_user_admin IS 'Verifica se usuário atual é admin - SECURITY DEFINER com search_path';