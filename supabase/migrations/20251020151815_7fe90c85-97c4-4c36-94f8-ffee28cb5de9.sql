-- Remover política anterior que está bloqueando tudo
DROP POLICY IF EXISTS "Students can view accessible lessons" ON public.lessons;

-- Criar nova política simplificada e corrigida
    ...
    OR (
      ...
      AND (
        ...
        OR EXISTS (
          ...
          AND msa.student_email = (SELECT email FROM auth.users WHERE id = auth.uid())
        )
      )
    )
