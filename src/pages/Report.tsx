...
const Report = () => {
  ...
  const categories = ['Produto fraudulento', 'Conteúdo pirata', 'Golpe/Burla', 'Informações enganosas', 'Violação de direitos autorais', 'Outro'];
  ...
  return <>
      <SEO title="Denuncie - Kambafy" description="Denuncie fraudes, burlas e conteúdos suspeitos na plataforma Kambafy" />
              ...
              <form onSubmit={handleSubmit} className="bg-card border border-border rounded-lg p-8">
                <div className="space-y-6">
                  <div>
                    ...
                    <Input id="reporterName" name="reporterName" value={formData.reporterName} onChange={handleChange} placeholder="Digite seu nome" className="mt-2" />
                  </div>

                  <div>
                    <Label htmlFor="reporterEmail">Seu Email *</Label>
                    <Input id="reporterEmail" name="reporterEmail" type="email" value={formData.reporterEmail} onChange={handleChange} placeholder="seu@email.com" className="mt-2" required />
                  ...
                  <div>
                    ...
                    <Input id="reportedUrl" name="reportedUrl" value={formData.reportedUrl} onChange={handleChange} placeholder="https://kambafy.com/produto/exemplo ou @username" className="mt-2" required />
                  </div>

                  <div>
                    <Label htmlFor="category">Categoria da Denúncia *</Label>
                    <select id="category" name="category" value={formData.category} onChange={handleChange} className="mt-2 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" required>
                  ...
                  <div>
                    ...
                    <Textarea id="description" name="description" value={formData.description} onChange={handleChange} placeholder="Descreva o problema em detalhes. Inclua evidências, capturas de tela ou qualquer informação relevante." className="mt-2 min-h-[150px]" required />
                    <p className="text-xs text-muted-foreground mt-1">
                      Mínimo 50 caracteres. Quanto mais detalhes, melhor.
                    </p>
                  </div>

                  <Button type="submit" className="w-full" disabled={isSubmitting || formData.description.length < 50}>
