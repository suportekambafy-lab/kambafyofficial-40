...
const categoryLabels: Record<Category, { label: string; color: string; icon: any }> = {
  duvidas: { label: "Dúvidas", color: "bg-blue-500", icon: MessageSquare },
  dicas: { label: "Dicas", color: "bg-green-500", icon: Sparkles },
  novidades: { label: "Novidades", color: "bg-purple-500", icon: Flame },
  produtos: { label: "Produtos", color: "bg-orange-500", icon: Award },
  marketing: { label: "Marketing", color: "bg-pink-500", icon: TrendingUp },
  tecnologia: { label: "Tecnologia", color: "bg-cyan-500", icon: Crown },
  geral: { label: "Geral", color: "bg-gray-500", icon: Users }
};
...
export default function SellerCommunity() {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      toast({ title: "Post criado!", description: "Publicado na comunidade" });
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      toast({ title: "Post deletado", description: "Removido com sucesso" });
      loadPosts();
    } catch (error) {
  ...
  const handleSharePost = (post: Post) => {
    if (navigator.share) {
      ...
      toast({ title: "Link copiado!", description: "Compartilhe com outros criadores" });
    }
  };
      ...
      <div className="space-y-6">
        ...
        >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 justify-between pt-4">
                    ...
                    <Button
                      ...
                      disabled={uploadProgress || !newPost.title || !newPost.content}
                    >
                      {uploadProgress ? (
                        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: "Membros", value: communityStats.totalMembers, icon: Users, color: "text-blue-500" },
              { label: "Posts", value: communityStats.totalPosts, icon: MessageSquare, color: "text-green-500" },
              { label: "Hoje", value: communityStats.todayPosts, icon: Flame, color: "text-orange-500" },
              { label: "Online", value: communityStats.activeNow, icon: Crown, color: "text-purple-500" }
            ].map((stat, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                return (
                  ...
                  >
                    <Card className={`overflow-hidden transition-all duration-300 ${
                      hoveredPost === post.id ? 'shadow-lg scale-[1.01]' : ''
                    }`}>
                        ...
                        <div className="flex items-center gap-4 mt-4 pt-4 border-t">
                          ...
                          <motion.button
                            ...
                            className={`p-2 rounded-full transition-colors ${
                              isSaved ? 'bg-primary/10 text-primary' : 'hover:bg-muted'
                            }`}
                          >
