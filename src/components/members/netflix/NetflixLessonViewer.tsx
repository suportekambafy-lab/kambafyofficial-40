...
import { ArrowLeft, Play, RotateCcw, SkipForward, ChevronDown, ChevronUp, Clock, CheckCircle2, FileText, Link as LinkIcon, Info } from 'lucide-react';
...
export function NetflixLessonViewer({
  ...
  return <div className="min-h-screen netflix-member-area" style={{
    ...
    color: 'hsl(40 20% 95%)'
      ...
      <main className="pt-16">
        ...
        <div className="relative w-full bg-black">
          {isNotYetReleased ? <div className="aspect-video lg:aspect-[21/9] xl:aspect-[2.4/1] 2xl:h-[70vh] 2xl:aspect-auto">
              <LessonReleaseTimer releaseDate={new Date(lesson.scheduled_at!)} lessonTitle={lesson.title} />
            </div> : hlsUrl || lesson.video_url || lesson.bunny_embed_url ? <div className="w-full aspect-video lg:aspect-[21/9] xl:aspect-[2.4/1] 2xl:h-[70vh] 2xl:aspect-auto relative">
              <VideoPlayer key={`${lesson.id}-${videoKey}`} hlsUrl={hlsUrl || undefined} embedUrl={lesson.bunny_embed_url || lesson.video_url || undefined} startTime={startTime} onTimeUpdate={onUpdateProgress && !isReplayMode ? (currentTime, duration) => {
            onUpdateProgress(lesson.id, currentTime, duration);
          } : undefined} onEnded={handleVideoEnd} />
              
              {/* Video End Overlay */}
              <AnimatePresence>
                {videoEnded && <motion.div initial={{
              opacity: 0
            }} animate={{
              opacity: 1
            }} exit={{
              opacity: 0
                  ...
                  delay: 0.1
                        ...
                        {nextLesson && <Button onClick={handleNextLesson} className="gap-2 h-12 px-6 bg-netflix-green hover:bg-netflix-green/90 text-black font-bold">
            ...
            </div> : <div className="aspect-video flex items-center justify-center bg-gradient-to-br from-amber-900/30 to-stone-900">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                {lesson.duration > 0 && <span className="flex items-center gap-1 text-sm text-white/50">
                    <Clock className="w-3.5 h-3.5" />
                    {formatDuration(lesson.duration)}
                  </span>}
                {currentProgress?.completed && <Badge className="bg-white/10 text-white/70 border-0 gap-1">
            ...
            <Tabs defaultValue="about" className="mb-8">
              <TabsList className="bg-white/5 border border-white/10 p-1 h-auto">
                ...
                {lesson.lesson_materials && (lesson.lesson_materials as any[]).length > 0 && <TabsTrigger value="materials" className="data-[state=active]:bg-netflix-green data-[state=active]:text-black text-white/70 gap-2">
                ...
                {lesson.complementary_links && (lesson.complementary_links as any[]).length > 0 && <TabsTrigger value="links" className="data-[state=active]:bg-netflix-green data-[state=active]:text-black text-white/70 gap-2">
              ...
              <TabsContent value="about" className="mt-4">
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  {lesson.description ? <p className="text-white/70 leading-relaxed whitespace-pre-wrap">
                      {lesson.description}
                    </p> : <p className="text-white/40 italic">Nenhuma descrição disponível para esta aula.</p>}
                </div>
              </TabsContent>

              {lesson.lesson_materials && (lesson.lesson_materials as any[]).length > 0 && <TabsContent value="materials" className="mt-4">
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    <div className="grid gap-3">
                      {(lesson.lesson_materials as any[]).map((material, i) => <a key={i} href={material.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors group">
              ...
              {lesson.complementary_links && (lesson.complementary_links as any[]).length > 0 && <TabsContent value="links" className="mt-4">
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    <div className="grid gap-3">
                      {(lesson.complementary_links as any[]).map((link, i) => <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors group">
                          <div className="w-10 h-10 rounded-lg bg-netflix-green/20 flex items-center justify-center">
            ...
            <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
              <button onClick={() => setShowLessonList(!showLessonList)} className="w-full px-5 py-4 flex items-center justify-between text-left hover:bg-white/5 transition-colors">
              ...
              <AnimatePresence>
                {showLessonList && <motion.div initial={{
                height: 0,
                opacity: 0
              }} animate={{
                height: 'auto',
                ...
                opacity: 0
                    ...
                    return <button key={l.id} onClick={() => !isActive && onNavigateLesson(l.id)} className={cn("w-full px-5 py-3 flex items-center gap-4 text-left transition-colors", isActive ? "bg-netflix-green/10 border-l-2 border-netflix-green" : "hover:bg-white/5")}>
                            {/* Thumbnail */}
                            <div className="relative w-24 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-white/10">
                              {getLessonThumbnail(l) ? <img src={getLessonThumbnail(l)} alt={l.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
                                  <Play className="w-4 h-4 text-white/30" />
                                </div>}
                              {progress > 0 && !isCompleted && <div className="absolute bottom-0 left-0 right-0 h-1 bg-black/50">
                                  <div className="h-full bg-netflix-green" style={{
                            width: `${progress}%`
                          }} />
                                </div>}
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {l.duration > 0 && <span className="text-xs text-white/40">
