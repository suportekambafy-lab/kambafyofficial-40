      ...
      <div className="mx-auto max-w-4xl px-6">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className="border border-gray-200 bg-white rounded-2xl overflow-hidden transition-all duration-300 hover:border-checkout-green/50 shadow-sm hover:shadow-md"
            >
              <button
                onClick={() => toggleFAQ(index)}
                className="w-full flex items-center justify-between p-6 text-left hover:bg-checkout-green/5 transition-colors"
              >
                <h3 className="text-base sm:text-lg font-semibold pr-4 text-gray-900">
                  {faq.question}
                </h3>
                {openIndex === index ? (
