    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          right: 0,
          zIndex: 20,
          background: boxColor ?? 'hsl(var(--checkout-green))',
          borderRadius: 4,
        }}
      />
    </section>
...
const TechOrbitDisplay = memo(function TechOrbitDisplay({
  ...
  return (
    <section className='relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-lg'>
      <span className='pointer-events-none whitespace-pre-wrap bg-gradient-to-b from-checkout-green to-checkout-green/60 bg-clip-text text-center text-7xl font-bold leading-none text-transparent'>
        {text}
      </span>
  ...
  return (
    <section className='max-md:w-full flex flex-col gap-4 w-96 mx-auto'>
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          <button
            className='bg-checkout-green hover:bg-checkout-green/90 relative group/btn w-full text-white rounded-md h-10 font-medium shadow-lg outline-hidden hover:cursor-pointer disabled:opacity-50 transition-all duration-200'
            type='submit'
            disabled={loading}
          >
        ...
        {textVariantButton && goTo && (
          <BoxReveal boxColor='hsl(var(--muted))' duration={0.3}>
            <section className='mt-4 text-center hover:cursor-pointer'>
              <button
                className='text-sm text-checkout-green hover:underline outline-hidden'
                onClick={goTo}
                type='button'
              >
                {textVariantButton}
...
const BottomGradient = () => {
  return (
    <>
      <span className='group-hover/btn:opacity-100 block transition duration-500 opacity-0 absolute h-px w-full -bottom-px inset-x-0 bg-gradient-to-r from-transparent via-checkout-green to-transparent' />
      <span className='group-hover/btn:opacity-100 blur-sm block transition duration-500 opacity-0 absolute h-px w-1/2 mx-auto -bottom-px inset-x-10 bg-gradient-to-r from-transparent via-checkout-green to-transparent' />
    </>
  );
};
