...
const RANK_COLORS = ['#4CAF50', '#00AEEF', '#FFC107', '#FF9800', '#F44336'];
...
export function ModernPaymentMethodsChart() {
  ...
  const fetchPaymentMethodsData = async () => {
    ...
    try {
      ...
      let query = supabase.from('orders').select('payment_method, product_id, order_bump_data').eq('status', 'completed').neq('payment_method', 'member_access');
      ...
      orders?.forEach(order => {
        ...
        const label = PAYMENT_METHOD_LABELS[method] || method.charAt(0).toUpperCase() + method.slice(1).replace(/_/g, ' ');
      ...
      const sortedMethods = Object.entries(methodCounts).map(([name, value]) => ({
        name,
        value,
  ...
  return <Card className="rounded-[18px] bg-card border-0 shadow-[0_2px_6px_rgba(0,0,0,0.05)] overflow-hidden h-full">
      <CardContent className="p-4">
          ...
          <div className="flex items-center gap-1.5">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="rounded-xl">
                {Object.entries(filterLabels).map(([key, label]) => <DropdownMenuItem key={key} onClick={() => setFilter(key as FilterOption)} className="cursor-pointer">
                    {label}
                  </DropdownMenuItem>)}
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <AnimatePresence mode="wait">
          ...
          opacity: 0
            ...
            </motion.div> : data.length === 0 ? <motion.div key="empty" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          opacity: 0
            ...
            </motion.div> : <motion.div key="chart" initial={{
          opacity: 0,
          scale: 0.8
        }} animate={{
          opacity: 1,
          ...
          ease: "easeOut"
              ...
              <DonutChart data={donutData} size={140} strokeWidth={28} animationDuration={0.8} animationDelayPerSegment={0.1} onSegmentHover={setHoveredSegment} centerContent={<div className="text-center">
                    {hoveredSegment ? <>
              ...
              {/* Legend */}
              <motion.div className="flex flex-wrap justify-center gap-1.5 mt-3" initial={{
            opacity: 0,
            y: 10
          }} animate={{
            opacity: 1,
            ...
            duration: 0.4
                ...
                {data.map((item, index) => <motion.div key={item.name} initial={{
              opacity: 0,
              scale: 0.8
            }} animate={{
              opacity: 1,
