...
export function AppHome() {
  ...
  } = useKambaLevels(stats.totalRevenue);
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      } = await supabase.from('products').select('id, name').eq('user_id', user.id).eq('status', 'Pendente').limit(5);
      ...
      const {
        data: recentOrders
      } = await supabase.from('orders').select('id, amount, currency, created_at, product_id').in('product_id', products.map(p => p.id)).eq('status', 'completed').gte('created_at', yesterday.toISOString()).order('created_at', {
        ascending: false
      }).limit(3);
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const {
        data: orders
      } = await supabase.from('orders').select('amount, seller_commission, currency, created_at, payment_method').in('product_id', productIds).eq('status', 'completed').neq('payment_method', 'member_access').order('created_at', {
        ascending: true
      });
      let totalRevenue = 0;
  ...
  const loadFinancialData = async (orders: any[], productIds: string[]) => {
    ...
    try {
      ...
      const {
        data: releases
      } = await supabase.from('payment_releases').select('order_id, amount, processed_at').eq('user_id', user.id);

      // Buscar withdrawal_requests
      const {
        data: withdrawals
      } = await supabase.from('withdrawal_requests').select('id, amount, status, created_at').eq('user_id', user.id).order('created_at', {
        ascending: false
      });
      const now = new Date();
      ...
      const totalWithdrawnAmount = withdrawals?.filter(w => w.status === 'aprovado').reduce((sum, w) => sum + parseFloat(w.amount?.toString() || '0'), 0) || 0;
  ...
  const renderContent = () => {
    if (showNotifications) {
      return <div className="p-4 space-y-4">
          ...
          {notifications.length === 0 ? <Card className="overflow-hidden border-none shadow-sm">
              ...
              {notifications.map(notification => <Card key={notification.id} className="overflow-hidden border-none shadow-sm">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${notification.type === 'success' ? 'bg-green-500/10' : notification.type === 'info' ? 'bg-blue-500/10' : 'bg-yellow-500/10'}`}>
                        <Bell className={`h-5 w-5 ${notification.type === 'success' ? 'text-green-600 dark:text-green-400' : notification.type === 'info' ? 'text-blue-600 dark:text-blue-400' : 'text-yellow-600 dark:text-yellow-400'}`} />
                      </div>
    ...
    switch (activeTab) {
      case 'products':
        return <div className="p-4 space-y-4">
            ...
            {loading ? <Card className="overflow-hidden border-none shadow-sm">
                <CardContent className="p-8 text-center">
                  <p className="text-muted-foreground">Carregando...</p>
                </CardContent>
              </Card> : products.length === 0 ? <Card className="overflow-hidden border-none shadow-sm">
                <CardContent className="p-8 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center mx-auto mb-4">
              ...
              <Card className="overflow-hidden border-l-4 border-primary shadow-sm bg-card">
                <CardContent className="p-6">
                  <div className="space-y-4">
                    ...
                    <Button onClick={() => setShowWithdrawalModal(true)} className="w-full mt-2" size="sm">
              ...
              <CardContent className="p-6">
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={salesData} margin={{
                    top: 10,
                    right: 10,
                    left: -20,
                    bottom: 10
      ...
      case 'profile':
        return <div className="p-4 space-y-4">
            ...
            <div className="flex justify-center pt-4 pb-2">
              <img src="/kambafy-logo-new.svg" alt="Kambafy" className="h-10 w-auto" />
            </div>
          </div>;
      default:
        return <div className="p-4 space-y-6">
            ...
            <div className="flex justify-center pt-2 pb-4">
              <img src="/kambafy-logo-new.svg" alt="Kambafy" className="h-12 w-auto" />
            </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {nextLevel && <span className="text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-sm min-w-[45px]" style={{
                      color: nextLevel?.color || '#FFD700'
                    }}>
                        {goalProgress.toFixed(0)}%
                      </span>
                    </div>
                    {currentLevel && <p className="text-xs text-muted-foreground mt-2">
            ...
            <Card className="overflow-hidden border-none shadow-sm">
              <CardContent className="p-6">
                ...
                <div className="space-y-2">
                  <Button variant="outline" className="w-full justify-start h-12 hover:bg-primary/5 hover:text-primary transition-colors" onClick={() => setActiveTab('stats')}>
                    <Wallet className="h-5 w-5 mr-3" />
                    <span className="font-medium">Ver financeiro</span>
                  </Button>
                  <Button variant="outline" className="w-full justify-start h-12 hover:bg-primary/5 hover:text-primary transition-colors" onClick={() => setActiveTab('products')}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-10 bg-background/80 backdrop-blur-md pb-safe">
        <div className="px-4 py-3">
          <div className="flex items-center justify-between gap-3 max-w-2xl mx-auto">
            {/* Back Button */}
            <button onClick={() => activeTab !== 'home' ? setActiveTab('home') : window.history.back()} className="w-12 h-12 rounded-full bg-card shadow-md flex items-center justify-center hover:shadow-lg transition-shadow flex-shrink-0">
            ...
            {/* Profile Button */}
            <button onClick={() => setActiveTab('profile')} className={`w-12 h-12 rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-shadow flex-shrink-0 overflow-hidden ${activeTab === 'profile' ? 'ring-2 ring-primary ring-offset-2' : ''}`}>
