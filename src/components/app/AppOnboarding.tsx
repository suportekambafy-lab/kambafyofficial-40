...
import { ChevronRight, Shield, Zap, Check, FastForward } from 'lucide-react';
...
const slides: OnboardingSlide[] = [{
  icon: <FastForward className="w-16 h-16 text-primary" />,
  ...
  return <div className="min-h-screen relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 p-6 flex justify-between items-center">
        ...
        x: 0
          ...
          <img alt="Kambafy" className="h-14 w-auto drop-shadow-lg" src="/lovable-uploads/c9a8c61f-5fe2-429f-9576-c13bb8fc42c3.png" />
        </motion.div>
        {!isLastSlide && <motion.div initial={{
        opacity: 0,
        x: 20
      }} animate={{
        opacity: 1,
        x: 0
            ...
            <Button variant="ghost" size="sm" onClick={handleSkip} className="text-foreground/70 hover:text-foreground hover:bg-foreground/10 backdrop-blur-sm">
              Pular
            </Button>
          </motion.div>}
      </div>
      ...
      <div className="relative z-10 p-6 space-y-6">
        ...
        <div className="flex justify-center gap-2.5">
          {slides.map((_, index) => <motion.button key={index} onClick={() => setCurrentSlide(index)} className={`h-2.5 rounded-full transition-all duration-300 ${index === currentSlide ? 'w-10 bg-primary shadow-lg' : 'w-2.5 bg-foreground/40 hover:bg-foreground/60'}`} whileHover={{
          scale: 1.2
        }} whileTap={{
          scale: 0.9
        }} />)}
        ...
        delay: 0.25
          ...
          <button onClick={handleNext} className="w-full h-14 bg-primary text-primary-foreground rounded-2xl font-bold text-[17px] flex items-center justify-center gap-2 shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
            {isLastSlide ? <>
