...
export default function VimeoMigration() {
  ...
  const executeMigration = async () => {
    try {
      ...
      console.log('🚀 Iniciando migração de vídeos do Cloudflare para Vimeo Pro...');

      const { data, error: invokeError } = await supabase.functions.invoke('migrate-to-vimeo', {
        body: {}
      });
    ...
    <Card className="w-full max-w-2xl mx-auto mt-8">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Migração para Vimeo Pro
        </CardTitle>
        <CardDescription>
          Migrar vídeos existentes do Cloudflare Stream para Vimeo Pro com domain whitelist
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg space-y-2">
          <p className="text-sm font-semibold text-blue-900 dark:text-blue-100">
            ℹ️ Sobre esta migração:
          </p>
          <ul className="text-xs text-blue-800 dark:text-blue-200 space-y-1 list-disc list-inside">
            <li>Migra vídeos do Cloudflare para Vimeo Pro</li>
            <li>Configura domain whitelist automaticamente</li>
            <li>Remove marca d'água do Vimeo (Vimeo Pro)</li>
            <li>Vídeos ficam privados e protegidos</li>
            <li>Processo pode levar 15-30 minutos</li>
          </ul>
        </div>
        ...
        >
          {loading ? (
            <>
              ...
              Migrando vídeos para Vimeo Pro...
            </>
          ) : (
            <>
              <Video className="mr-2 h-4 w-4" />
              Iniciar Migração para Vimeo Pro
            </>
          )}
        </Button>
        ...
        {loading && (
          <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
            ...
            <p className="text-xs text-blue-800 dark:text-blue-200 mt-2">
              Os vídeos estão sendo transferidos para o Vimeo e configurados com proteção máxima
            </p>
          </div>
        )}
        ...
        <div className="bg-yellow-50 dark:bg-yellow-950 p-4 rounded-lg space-y-2 text-xs">
          ...
          <ul className="text-yellow-800 dark:text-yellow-200 space-y-1 list-disc list-inside">
            <li>Os vídeos do Cloudflare NÃO serão deletados automaticamente</li>
            <li>Você pode deletá-los manualmente depois de confirmar que tudo funciona</li>
            <li>O Vimeo Pro custa $9/mês (vs Cloudflare $15-20/mês)</li>
            <li>Bandwidth ilimitado no Vimeo (vs pago por GB no Cloudflare)</li>
          </ul>
        </div>
      </CardContent>
    </Card>
